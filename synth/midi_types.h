...
public:
    ...
    MIDIevent()
        : position(1), type(0x09), data1(0), data2(0), data3(0) {}

    // Constructeur pratique avec paramètres
    MIDIevent(uint32_t pos, uint8_t t, uint8_t d1, uint8_t d2, uint8_t d3)
        : position(pos), type(t), data1(d1), data2(d2), data3(d3) {}

    // Destructeur par défaut (ne fait rien de spécial)
    ~MIDIevent() = default;

    // Données MIDI
    uint32_t position; // Position dans la piste
    uint8_t type;      // Type d'événement MIDI
    uint8_t data1;     // Premier octet de données
    uint8_t data2;     // Deuxième octet de données
    uint8_t data3;     // Troisième octet de données
};
